...
\date{Third revised version, August 24, 2026}
...
A real matrix is \emph{totally nonnegative} if every minor determinant is nonnegative.  If a full-row-rank $r\times n$ matrix has nonnegative maximal minors, its column matroid is a positroid; equivalently, its row span belongs to the totally nonnegative Grassmannian.  Throughout the low-rank classification, however, we impose the stronger hypothesis that the Toeplitz representative itself is totally nonnegative in \emph{all} minor orders, rather than merely requiring its maximal minors to be nonnegative.
...
\]
For two nonzero columns $c_i,c_j$, the change in the first moment satisfies the subtraction-free identity
\[
 s_is_j(u_j-u_i)
...
It remains to justify uniqueness from the positroid support, rather than from a chosen representing matrix.  The basis set determines the rank-three matroid.  Its loops are exactly the elements contained in no basis.  For two nonloops $e,f$, the pair is parallel if and only if no basis contains both: every independent pair in a matroid extends to a basis.  Hence all parallel classes, and in particular the ordered endpoint classes $P$ and $Q$, are determined.  After simplification, a subset $X$ of at least three elements has rank two if and only if every three-element subset of $X$ is a nonbasis.  Therefore the maximal rank-two flats of the simplified matroid are determined by the basis support.  The preceding necessity argument identifies those flats with the ordinary intervals in $\mathcal H$.  Thus $\mathcal D=(L_0,R_0;P,Q;\mathcal H)$ is uniquely recovered from the positroid.
\end{proof}
...
\]
then columns $1,\ldots,p$ form an initial parallel class.  The first edge of the simplified chain has slope $R$.  Subsequent finite slopes can be prescribed by \eqref{eq:recurrence}.
...
\]
the slope from the first projective class to the next is $R$.  The next finite slope is given by \eqref{eq:slopeformula} and is larger than $r_{p+1}>R$.  For (2),
\[
 Q_k=(R,r_{k-1}R),
 \qquad
...
\end{aligned}
\end{equation}
The only nonsingleton parallel classes are the first and last fibers in \eqref{eq:raw-simple-map}, and the nontrivial maximal constant runs of the simplified edge-slope sequence are exactly the intervals in $\mathcal H$.  The resulting matrix is full-row-rank and totally nonnegative.
\end{lemma}
...
\[
 ...
 \qquad 0\le j\le d+2.
\]
Suppose that $b$ is strictly log-concave, with the zero boundary convention.  Lemma~\ref{lem:TN2criterion} then makes $C(b)$ $\TN_2$ with all nonstructural $2\times2$ minors positive.  The boundary support patterns and the strict nonstructural minors exclude parallel columns, so Lemma~\ref{lem:moment-separation} gives strictly increasing moment coordinates
\[
...
\begin{equation}\label{eq:Zpattern}
 ...
 \qquad(1\le t\le d-1).
\end{equation}
All nonstructural minors of order at most two are positive.
\end{theorem}
...
\]
Monotonicity of the $j_\ell$ makes this family equivalent to its last inequality, which is \eqref{eq:Schubert-condition}.  The cardinality form and the uniform cases follow immediately.
\end{proof}
...
\bibitem{McDaniel}
C. McDaniel,
\emph{A Hessian criterion for totally positive Toeplitz matrices and a new proof of Cattani's theorem in two variables},
arXiv:2507.15043 (2025).
...
Proceedings of the International Congress of Mathematicians 2026, advance online publication;
\url{https://doi.org/10.1137/25M1806727}; arXiv:2509.25163.
